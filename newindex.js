...
function animateParticles() {
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    if (distance < 1000) { // Adjust the range parameter as needed
      particleMaterial.map = sampleQRTexture;
      particleMaterial.needsUpdate = true;
    } else {
      particleMaterial.map = null;
      particleMaterial.needsUpdate = true;
    }
...
function animateText() {
  ...
  const timings = [
    { element: text1, delay: startTime },
    { element: text2, delay: startTime + (fadeTime + showTime + intervalTime) * 1 },
    { element: text3, delay: startTime + (fadeTime + showTime + intervalTime) * 2 },
    { element: text4, delay: startTime + (fadeTime + showTime + intervalTime) * 3 },
  ];

  timings.forEach(({ element, delay }, index) => {
    setTimeout(() => fadeIn(element), delay);
    setTimeout(() => fadeOut(element), delay + fadeTime + showTime);
  });
}

animateText();
setInterval(animateText, 120000); // Repeat the animation every 60 seconds
